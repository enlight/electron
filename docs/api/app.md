...
If `callback` is `null` the previously set custom Jump List (if any) will be
replaced by the standard Jump List for the app (managed by Windows). If
`callback` is a function then it will be called immediately with
`callback(minItems, removedItems)` and must return an array of
`JumpListCategory` objects. `minItems` indicates the the minimum number of
items that will be shown in the Jump List (more details on
[MSDN][JumpListBeginListMSDN]). `removedItems` is an array of `JumpListItem`
objects that correspond to items that the user has explicitely removed from the
Jump List, these items must not appear in any of the custom categories returned
by the callback function, Windows will not display any custom category that
      ...
      items: [
        {
          ...
          program: process.execPath, arguments: '--run-tool-a',
          icon: process.execPath, iconIndex: 0,
          description: 'Runs Tool A'
        },
        {
          type: 'task', title: 'Tool B',
          program: process.execPath, arguments: '--run-tool-b',
          icon: process.execPath, iconIndex: 0,
      ...
      items: [
        {
          ...
          program: process.execPath, arguments: '--new-project',
